...
public class HTMLUtility {
	...
	public static String getList1(String name, String selectedVal, Map<Integer, String> map) {
		...
		System.out.println(selectedVal + "hhhhhhhhhhhhhhhiiiiiiiiiiiiiiiiii--==");
		...
		for(Integer key : keys) {
			val = map.get(key);
			// Convert key to String for camparison and value attribute
			String keyString = val.toString();
			System.out.println(keyString +"1234567898765000----======");
			
			if (keyString.trim().equals(selectedVal)) {
				
				System.out.println(selectedVal + "1234567876543--==)");
				//mark the option as selected if it matches the selecvtedVal
	...
	public static String getList2(String name, String selectedVal, Map<Integer, String> map) {
		...
		StringBuilder sb = new StringBuilder("<select style='width: 203px;  height: 23px;'  class='form-control' name='" + name + "'>");
		...
		if (select) {
			...
			sb.append(
					"<option  style='width: 203px;  height: 30px;' class='dropdown-item' selected value=''>------Select a---- " + name + "--------</option>");
